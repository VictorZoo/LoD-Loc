...
class Location2DRecall(torchmetrics.MeanMetric):
    ...
    def update(self, pred, data):
        error = location_error(pred[self.key][...,1:], data["pose_GT"][...,3:]) #XYZ
        super().update((error <= self.threshold).float())

class AllRecall_6DoF(torchmetrics.MeanMetric):
...
class AllRecall(torchmetrics.MeanMetric):
    ...
    def update(self, pred, data):
        angle = angle_error(pred[self.key][..., 0], data["pose_GT"][..., 2])
        xyz_error = location_error(pred[self.key][..., 1:], data["pose_GT"][...,3:])
        flag = torch.logical_and(angle <= self.angle_threshold, xyz_error <= self.xyz_threshold)
        super().update(flag.float())
...
class Location2DRecall_xy(torchmetrics.MeanMetric):
    ...
    def update(self, pred, data):
        error = location_error(pred[self.key][...,1:-1], data["pose_GT"][...,3:-1]) #XYZ
        super().update((error <= self.threshold).float())
...
class AngleRecall(torchmetrics.MeanMetric):
    ...
    def update(self, pred, data):
        error = angle_error(pred[self.key][..., 0], data["pose_GT"][..., 2])
        super().update((error <= self.threshold).float())
...
class AngleError(MeanMetricWithRecall):
    ...
    def update(self, pred, data):
        value = angle_error(pred[self.key][...,0], data["pose_GT"][..., 2])
        if value.numel():
            self.value.append(value)

class RError(MeanMetricWithRecall):
    def __init__(self, key="rxyz_max"):
        super().__init__()
        self.key = key

    def update(self, pred, data):
        e_t, value = error_4x4(pred[self.key], data["pose_GT_4x4"].to(pred[self.key]))
        if value.numel():
            self.value.append(value)
...
class Location2DError(MeanMetricWithRecall):
    ...
    def update(self, pred, data):
        value = location_error(pred[self.key][...,1:], data["pose_GT"][...,3:])
        if value.numel():
            self.value.append(value)
...
class Location2DError_x(MeanMetricWithRecall):
    ...
    def update(self, pred, data):
        value = location_error_single(pred[self.key][...,1], data["pose_GT"][...,3])
...
class Location2DError_y(MeanMetricWithRecall):
    ...
    def update(self, pred, data):
        value = location_error_single(pred[self.key][...,2], data["pose_GT"][...,4])
        # print("Location error: ", value)
        if value.numel():
            self.value.append(value)
...
class Location2DError_z(MeanMetricWithRecall):
    ...
    def update(self, pred, data):
        value = location_error_single(pred[self.key][...,3], data["pose_GT"][...,5])
        # print("Location error: ", value)
        if value.numel():
            self.value.append(value)
