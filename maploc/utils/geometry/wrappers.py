...
from .transforms import axis_angle_to_quaternion, quaternion_to_axis_angle,\
    axis_angle_to_matrix, matrix_to_quaternion, quaternion_to_matrix
...
class TensorWrapper:
    ...
    def unsqueeze(self, n):
        return self.__class__(self._data.unsqueeze(n))

    def expand(self, *args):
        return self.__class__(self._data.expand(args + tuple(self._data.shape[len(self.shape):])))

    def reshape(self, *args):
        return self.__class__(self._data.reshape(args + tuple(self._data.shape[len(self.shape):])))

    def view(self, *args):
        return self.__class__(self._data.view(args + tuple(self._data.shape[len(self.shape):])))

    def permute(self, *args):
        return self.__class__(self._data.permute(args + tuple([len(self.shape)])))
...
class Pose(TensorWrapper):
    ...
    @classmethod
    @autocast
    def from_quat(cls, quat: torch.Tensor, t: torch.Tensor):
        ...
        assert t.shape[-1] == 3
        assert quat.shape[:-1] == t.shape[:-1]
        #return cls.from_Rt(so3exp_map(aa), t)
        return cls.from_Rt(quaternion_to_matrix(quat), t)
    ...
    @property
    def quat_transfrom_mat(self) -> torch.Tensor:
        quaternions = matrix_to_quaternion(self.R)
