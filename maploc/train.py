...
def train(cfg: DictConfig, job_id: Optional[int] = None):
    ...
    init_checkpoint_path = cfg.training.get("finetune_from_checkpoint") # 改
    if init_checkpoint_path is not None:
        ...
        model = GenericModule(cfg) # Get Model
    ...
    ) # save checkpont parameter epoch
    ...
    strategy = 'auto'
    if cfg.experiment.gpus > 1: #config for GPU
    ...
    data = data_modules[cfg.data.get("name", "UAVD4L-LoD")](cfg.data)

    tb_args = {"name": cfg.experiment.name, "version": ""}
    tb = pl.loggers.TensorBoardLogger(EXPERIMENTS_PATH, **tb_args) # Logger config
    ...
    trainer.fit(model, data, ckpt_path=last_checkpoint_path) #改


@hydra.main(
    config_path=osp.join(osp.dirname(__file__), "conf"), config_name="LoD-Loc"
)
